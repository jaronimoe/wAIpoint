...
- **If the CLI says no tracker repo is configured, or that the repo has no `waipoint-data.json`**, stop and tell the user; setting up the data repo is theirs to do with `waipoint init`. Don't set `WAIPOINT_REPO`, edit the config file, create the marker, or run `waipoint init` yourself to get past it — the check exists to stop writes landing in the wrong repo.
